...
def create_net(env_id=2, task='segmentation'):
    ...
    if task == 'monodepth':
        ...
        net = ailia.Net(MODEL_MONODEPTH_PATH, WEIGHT_MONODEPTH_PATH,
                        env_id=env_id, memory_mode=mem_mode)

    elif task == 'segmentation':
        check_and_download_models(WEIGHT_SEGMENTATION_PATH, MODEL_SEGMENTATION_PATH, REMOTE_PATH)
        net = ailia.Net(MODEL_SEGMENTATION_PATH, WEIGHT_SEGMENTATION_PATH,
                        env_id=env_id, memory_mode=mem_mode)

    return net


def infer_frame(net, frame, task='segmentation'):
    """
    1フレーム推論API
    """

    # BGR → RGB + normalize
    img_raw = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) / 255.0
    ...
    # 後処理
    result = postprocess(prediction, img_raw)
