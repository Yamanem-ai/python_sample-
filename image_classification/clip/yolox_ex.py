...
MODEL_PARAMS = {'yolox_nano': {'input_shape': [416, 416]},
                ...
                'yolox_model': {'input_shape': [640, 640]}}
...
COCO_CATEGORY = [
    ...
    "scissors", "teddy bear", "hair drier", "toothbrush", "umbrella2"
]
...
def detect_frame(detector, frame, target_classes=None):
    img, ratio = preprocess(frame, (HEIGHT, WIDTH))
    output = detector.predict(img[None, :, :, :])

    predictions = postprocess(output, (HEIGHT, WIDTH))[0]
    objs = predictions_to_object(predictions, frame, ratio, args.iou, args.threshold)

    if target_classes is not None:
        objs = [
            o for o in objs
            if COCO_CATEGORY[o.category] in target_classes
        ]
    print("detected:", len(objs))
    for obj in objs:
        label = COCO_CATEGORY[int(obj.category)]
...
def recognize_from_image(detector):
    ...
    for image_path in args.input:
        ...
        #推論前に
        logger.info(f'raw_img shape: {raw_img.shape}')
        logger.info(f'img shape: {img.shape}')
        logger.info(f'img dtype: {img.dtype}')
        logger.info(f'img max: {np.max(img)}, min: {np.min(img)}')
