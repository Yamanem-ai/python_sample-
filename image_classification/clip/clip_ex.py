...
def create_clip(model_type='ViTB32', env_id=0):
    dic_model = {
        'ViTB32': (
    ...
    (WEIGHT_TEXT_PATH, MODEL_TEXT_PATH) = dic_model[model_type]
    ...
    if "FP16" in ailia.get_environment(env_id).props \
        ...
        logger.warning(
            'CLIP does not work correctly on FP16. '
            'Fallback to CPU.'
        )
        env_id = 0
    ...
    return net_image, net_text


def create_text_features(net_text, text_inputs):
    """
    text_inputs:
        [
    ...
    text_feature = predict_text_feature(
        net_text,
        text_inputs
    )
...
def predict_clip(net_image, img, text_feature, text_inputs=None):
    """
    Parameters
    ----------
    net_image:
    ...
    if text_inputs is not None:
        result['best_text'] = text_inputs[best_idx]
...
def recognize_from_image(net_image, net_text):
    ...
    if desc_file:
        ...
        text_inputs = [f"a person wearing {c}" for c in ("headphones", "earphones", "earbuds", "headphones")]
