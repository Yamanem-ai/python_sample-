...
def initialize_clap():
    ...
    check_and_download_models(
        CLAP_AUDIO_WEIGHT_PATH,
        CLAP_AUDIO_MODEL_PATH,
        REMOTE_PATH
    )
    
    check_and_download_models(
        CLAP_TEXT_PROJECTION_WEIGHT_PATH,
        CLAP_TEXT_PROJECTION_MODEL_PATH,
        REMOTE_PATH
    )
    
    check_and_download_models(
        CLAP_TEXT_ROBERTAMODEL_WEIGHT_PATH,
        CLAP_TEXT_ROBERTAMODEL_MODEL_PATH,
        REMOTE_PATH
    )
    ...
    if not args.onnx:
        ...
        net_text_branch = ailia.Net(
            CLAP_TEXT_ROBERTAMODEL_MODEL_PATH,
            CLAP_TEXT_ROBERTAMODEL_WEIGHT_PATH,
            env_id=args.env_id
        )

        net_text_projection = ailia.Net(
            CLAP_TEXT_PROJECTION_MODEL_PATH,
            CLAP_TEXT_PROJECTION_WEIGHT_PATH,
            env_id=args.env_id
        )
    ...
    return {
        "net_audio": net_audio,
        "net_text_branch": net_text_branch,
        "net_text_projection": net_text_projection
    }

def authenticate_whistle(
    net_audio,
    registered_audio_path="open.m4a",
    threshold=0.8
):

    # microphone input
    record_audio("mic_input.wav")

    # embedding
    registered_embedding = infer_audio(
        net_audio,
        registered_audio_path
    )

    current_embedding = infer_audio(
        net_audio,
        "mic_input.wav"
    )
