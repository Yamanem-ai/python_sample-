...
def frame_vis_generator(frame, bboxes, local_ids, global_ids):
    for i, local_id in enumerate(local_ids):
        color = vis_colors[int(local_id) % num_colors]
...
def recognize_from_video(camera_id, video_file, tracking_queue, shared_global_id_map):
    ...
    while True:
        ...
        if frame_shown and cv2.getWindowProperty(window_name, cv2.WND_PROP_VISIBLE) == 0:
            break
        ...
        for t in tracker.tracks:
            ...
            if len(t.features) == 0:
                continue
            feature = np.mean(t.features, axis=0)
            ...
            if tlwh[2] * tlwh[3] > min_box_area and not vertical:
                ...
                gid = shared_global_id_map.get(key)

                display_id = gid if gid is not None else tid

                online_local_ids.append(tid)
                online_global_ids.append(display_id)

        frame_idx += 1

        res_img = frame_vis_generator(
            frame,
            online_tlwhs,
            online_local_ids,
            online_global_ids
