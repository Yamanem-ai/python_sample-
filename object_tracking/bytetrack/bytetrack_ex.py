...
parser.add_argument(
    '-m', '--model_type', default='mot17_tiny',
...
def create_bytetrack(
    model_type='mot17_tiny',
    env_id=2,
    track_thresh=0.5,
    track_buffer=30,
    match_thresh=0.8,
    ...
    check_and_download_models(
        weight_path,
        model_path,
        REMOTE_PATH if model_type.startswith('mot')
        else REMOTE_YOLOX_PATH
    )
    ...
    tracker = BYTETracker(
        ...
        frame_rate=30,
        mot20=(model_type == 'mot20_x')
    )

    return net, tracker

def update_bytetrack(
    net,
    tracker,
    frame,
    min_box_area=10,
