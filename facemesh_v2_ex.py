...
from blendshape import face_blendshapes, plot_face_blendshapes_bar_graph, Labels
...
def initialize_models():
    ...
    check_and_download_models(
        WEIGHT_BLENDSHAPE_PATH,
        MODEL_BLENDSHAPE_PATH,
        REMOTE_PATH
    )
    ...
    return {
        "net": net,
        "det_net": det_net,
        "blendshape": blendshape_net,
    }

    return models

def process_frame(models, frame):
...
def predict(models, img):
    ...
    for box in boxes:
        ...
        roi = ROI(
            ...
            angle)
        input_img, roi, pad = preprocess(img, roi)
        ...
        if not args.onnx:
            ...
            output = net.run(None, {'input_12': input_img})
